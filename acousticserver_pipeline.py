...
if __name__ == '__main__':
    ...
    parser =\
        argparse.ArgumentParser(description='Script to run facial feature '
                                'extraction from video interviews.')
    parser.add_argument(dest='read_dir', type=str,
                        help=('Path where videos to be analyzed by this'
                              ' script are stored'))
            ...
            try:
                ...
                if (len(listfiles)<3) & (len(listfiles)> 0):
                    ...
                    if len(listfiles)==2:
                        ...
                        cmd = 'docker run -v ' + str(path2file) + ':/data ' 
                        cmd = cmd +  '-v ' +str(path2results) + ':/results ' + 'acoustic.img ' 
                        cmd = cmd + 'python MainCode4FeatureExtraction_PREDICT.py /results '
                        cmd = cmd + str(nfiles)+ ' ' + file1 + ' ' + file2
                        
                        print(cmd)
                        
                        print('Ready to docker')
                        #run docker
                        os.system(cmd)
                        

                        print('Pass docker')
                        #As final step, we move and rename the files need and remove them from the intermediate
                        # folder results
                        ...
                        shutil.copy(os.path.join(path2results, 'S2GlobalFeat.csv'), \
                                    os.path.join(final_dir, sufix_name1 + '-' + sufix_name2 + '-openInterview_AcousticFeatures_Global_S2-'+ sufix_name4 + '.csv'))
                            
                        shutil.copy(os.path.join(path2results, 'S1GlobalFeat.csv'), \
                                        os.path.join(final_dir, sufix_name1 + '-' + sufix_name2 + '-openInterview_AcousticFeatures_Global_S1-'+ sufix_name4 + '.csv'))
                                                    
                        shutil.copy(os.path.join(path2results, 'LLDfeat_S1S2.csv'), \
                                        os.path.join(final_dir, sufix_name1 + '-' + sufix_name2 + '-openInterview_AcousticFeatures_LLD_S1S2-'+ sufix_name4 + '.csv'))
                        ...
                        print('FILES - only 1')
                        os.rename(os.path.join(path2file,listfiles[0] ), os.path.join(path2file,'yfirst_audiofile.m4a'))
                        file1 = os.path.join('/data', 'yfirst_audiofile.m4a') 
                        nfiles=1
                        
                        print('ALLFILES:', path2file, path2results, str(nfiles), file1)
                        
                        cmd = 'docker run -v ' + str(path2file) + ':/data ' 
                        cmd = cmd +  '-v ' +str(path2results) + ':/results ' + 'acoustic.img ' 
                        cmd = cmd + 'python MainCode4FeatureExtraction_PREDICT.py /results '
                        cmd = cmd + str(nfiles)+ ' ' + file1 
                        ...
                        shutil.copy(os.path.join(path2results, 'S1GlobalFeat.csv'), \
                                    os.path.join(final_dir, sufix_name + '_S1GlobalFeat.csv'))
                  
                        
                        shutil.copy(os.path.join(path2results, 'LLDfeat_S1S2.csv'), \
                                    os.path.join(final_dir, sufix_name + '_LLDfeat_S1S2.csv'))
